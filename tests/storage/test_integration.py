...
class TestCompleteScrapeWorkflow:
    ...
    def test_complete_workflow(self, db: Database):
        ...
        pages = [Page(page_id=i, namespace=0, title=f"Page{i}", is_redirect=False) for i in range(1, 101)]
...
class TestIncrementalUpdate:
    ...
    def test_incremental_update(self, db: Database):
        ...
        page1_count = conn.execute("SELECT COUNT(*) FROM revisions WHERE page_id = 1").fetchone()[0]
...
class TestForeignKeyEnforcement:
    ...
    def test_cascade_delete_removes_revisions(self, db: Database):
        ...
        count = conn.execute("SELECT COUNT(*) FROM revisions WHERE page_id = 1").fetchone()[0]
        assert count == 5
